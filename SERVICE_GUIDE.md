...
| 경로 | 역할 |
|--------|------|
| `src/lib/ui/` | 재사용 가능한 순수 UI 컴포넌트 (FileUploadZone, ProgressBar, Button 등) |
| `src/lib/pdf/processors/` | PDF 도구별 처리 함수 (순수 함수, UI 무관) |
| `src/app/pdf/[locale]/(tools)/_components/` | 도구 페이지 전용 컴포넌트 |
...
```typescript
// src/lib/pdf/processors/merge.ts
...
```typescript
// src/lib/i18n/config.ts → Dictionary 타입에 추가
...
```typescript
// src/app/pdf/[locale]/(tools)/_components/tools/merge-tool.tsx
"use client";

import { useState } from "react";
import { FileList, Button } from "@/lib/ui";
...
```typescript
// 추후: src/workers/pdf-processor.worker.ts
```
...
```typescript
// src/lib/pdf/processors/__tests__/merge.test.ts
...
1. [ ] `src/lib/pdf/processors/{slug}.ts` — 프로세서 함수 작성
2. [ ] `src/lib/pdf/processors/index.ts` — 레지스트리에 등록
3. [ ] `src/app/pdf/[locale]/(tools)/_components/tools/{slug}-tool.tsx` — 도구 UI 작성
4. [ ] 번역 키 추가 (43개 언어)
