...
import { AnnotateListPanel } from "./annotate-list-panel";
import { AnnotatePropertiesPanel } from "./annotate-properties-panel";
...
const AnnotatePageCanvas = dynamic(
  () =>
    import("./annotate-page-canvas").then((m) => ({
      default: m.AnnotatePageCanvas,
    })),
  {
    ssr: false,
    loading: () => (
...
interface AnnotateLayoutProps {
  ...
  onAnnotationsChange: (annotations: AnnotateElement[]) => void;
  onChangeFile: () => void;
}
...
export function AnnotateLayout({
  ...
  const { state, dispatch, undo, redo } = useAnnotateStore();
  ...
  const clipboardRef = useRef<AnnotateElement | null>(null);
          ...
          <div className="hidden w-[160px] shrink-0 flex-col overflow-hidden border-r border-border bg-background sm:flex lg:w-[190px]">
            ...
            <div className="flex flex-1 flex-col items-center gap-2 overflow-y-auto px-2 py-4">
              {pages.map((page, idx) => {
                ...
                const count = state.annotations.filter((a) => a.pageIndex === idx).length;
                ...
                {pages.map((page, idx) => (
                  ...
                  >
                    <AnnotatePageCanvas
                      page={page}
                      pageIndex={idx}
                      annotations={state.annotations.filter((a) => a.pageIndex === idx)}
                      state={state}
                      dispatch={dispatch}
                      scale={scale}
