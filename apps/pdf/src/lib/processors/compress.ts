...
async function compressImageMode(
  ...
  for (const [ref, obj] of context.enumerateIndirectObjects()) {
    ...
    imageEntries.push({ ref, stream: obj, w, h });
  }

  for (let i = 0; i < imageEntries.length; i++) {
    const { ref, stream, w: origW, h: origH } = imageEntries[i];

    const result = await recompressJpeg(
      stream.getContents(),
      config.quality,
      config.maxDimension,
    );
    ...
    if (!result) continue;
    if (result.bytes.length >= stream.getContentsSize()) continue;

    const newDict = stream.dict;

    if (result.width !== origW || result.height !== origH) {
...
const compressPdf: ProcessorFn = async (files, options, onProgress) => {
  ...
  if (files.length === 1) {
    const { bytes, pageCount } = await compressSinglePdf(
      files[0],
      level,
      mode,
      onProgress,
