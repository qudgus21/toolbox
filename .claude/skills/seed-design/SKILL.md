---
name: seed-design
description: 경쟁 서비스 URL/스크린샷을 분석하여 디자인 시스템과 UI를 프로젝트 내 해당 앱에 복제. Next.js + Tailwind CSS + Framer Motion 기반.
disable-model-invocation: false
allowed-tools: Read, Write, Edit, Glob, Grep, Bash, WebFetch, Task
user-invocable: true
---

# Seed Design — UI Clone Skill

너는 세계 최고의 **Full-stack UI Engineer**이자 **Design System Architect**야.
경쟁 서비스의 URL이나 스크린샷을 분석해서, ToolBox 프로젝트 내에 Next.js + Tailwind CSS + Framer Motion 기반 코드로 재현하는 것이 네 임무야.
...
- **구조**: 단일 Next.js 프로젝트
- **앱 라우트**: `src/app/{domain}/` — 도메인별 라우트 그룹
- **공유 라이브러리**: `src/lib/ui/`, `src/lib/design-tokens/`, `src/lib/i18n/`, `src/lib/seo/` 등
- **배포**: 단일 도메인 내 앱별 경로 (`pdf.toolbox.com`, `video.toolbox.com` 등)
...
```
Glob: src/app/{domain}/**/*.tsx
Glob: src/lib/ui/*.tsx
Glob: src/lib/design-tokens/tokens/*.ts
```
...
| 조건 | 모드 | 설명 |
|------|------|------|
| 공유 라이브러리가 아직 없음 | **Foundation 모드** | 디자인 토큰 + 공유 UI + 앱 페이지 전체 생성 |
| 공유 라이브러리 존재, 앱이 비어있음 | **App Init 모드** | 기존 디자인 시스템 활용, 앱 초기화 |
| 공유 라이브러리 + 앱 모두 존재 | **Page Add 모드** | 기존 앱에 새 도구 페이지 추가 |
...
|-------|-----------|----------|----------|
| Phase 1: 분석 | O | O | O |
| Phase 2: 디자인 시스템 | O (src/lib/) | SKIP | SKIP |
...
### Step 2-2: 디자인 토큰 (src/styles/tokens.css)
...
### Step 2-3: 공유 UI 컴포넌트 (src/lib/ui)

분석에서 식별된 공통 컴포넌트를 `src/lib/ui/`에 생성.
...
1. `Glob("src/lib/ui/components/*.tsx")`로 기존 목록 확인
2. 분석에서 필요한 것 중 **없는 것만** 새로 생성
3. 기존 컴포넌트는 **절대 수정하지 않는다** (variant 추가는 예외)

### 컴포넌트 작성 규칙

1. **파일**: `src/lib/ui/{component-name}.tsx`
...
```tsx
// src/app/{domain}/app/[locale]/page.tsx
import { Header, Footer, ToolCard } from '@/lib/ui';
...
### 생성된 파일
- `src/lib/ui/...` — 공유 UI 컴포넌트 (N개)
- `src/app/{domain}/...` — 앱 페이지 및 컴포넌트
...
- 앱 간 직접 import 금지 (반드시 `src/lib/` 통해서)
- 공유 컴포넌트는 `src/lib/ui/`에, 앱 전용은 `src/app/{domain}/_components/`에
- 디자인 토큰 변경은 모든 앱 라우트에 영향 → 신중하게
