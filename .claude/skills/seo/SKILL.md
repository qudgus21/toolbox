...
- **i18n**: `@/lib/i18n` — 43개 로케일, `[locale]` 동적 세그먼트
...
| 인자 | 대상 |
|------|------|
| `{앱이름}` (예: `pdf`, `video`, `text`, `image`) | 해당 앱 라우트 (`src/app/{앱이름}/`) |
| 인자 없음 | `src/app/` 하위의 첫 번째 앱 (현재: PDF) |
...
#### Step 2: `src/lib/seo/` 라이브러리 구축

Fix 실행 시 `src/lib/seo/`에 공유 유틸리티를 생성/업데이트한다:
...
#### Step 1: PSI API로 프로덕션 측정 (권장)

**항상 `.env.local`의 `PSI_API_KEY`를 사용한다** — 키 없이 호출하면 일일 쿼터가 즉시 초과된다.

```bash
PSI_KEY=$(grep PSI_API_KEY .env.local | cut -d= -f2)
curl -s "https://www.googleapis.com/pagespeedonline/v5/runPagespeed?url={URL}&strategy=mobile&category=seo&category=performance&category=accessibility&category=best-practices&key=${PSI_KEY}"
```

프로덕션 URL을 직접 측정하므로 로컬 서버 불필요. 모바일/데스크톱 모두 측정한다.

#### Step 2: 로컬 Lighthouse (대안)

PSI API를 사용할 수 없는 경우에만 로컬 방식을 사용한다:
