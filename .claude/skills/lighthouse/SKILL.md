...
- **앱 라우트**: 5개 앱 — 각 앱별 독립 라우트
  - `src/app/pdf/` — PDF 도구 (47개)
  - `src/app/image/` — 이미지 도구 (58개+)
  - `src/app/text/` — 텍스트 도구 (28개+)
  - `src/app/converter/` — 변환 도구 (40개+)
  - `src/app/calculator/` — 계산기 도구 (50개+)
- **랜딩 페이지**: `src/app/[locale]/(landing)/` — 홈, about, blog, contact, faq 등
...
| `image` | Image 앱 홈 + 주요 도구 페이지 감사 |
| `text` | Text 앱 홈 + 주요 도구 페이지 감사 |
| `converter` | Converter 앱 홈 + 주요 도구 페이지 감사 |
| `calculator` | Calculator 앱 홈 + 주요 도구 페이지 감사 |
| `landing` | 랜딩 홈 + about, blog, faq 등 감사 |
| `all` | 5개 앱 전체 + 랜딩 순차 감사 |
| `https://toolpop.org/pdf/ko/merge` | 해당 앱 홈 + 지정 URL 감사 |
| (인자 없음) | 사용자에게 앱 선택 요청 |

**앱 홈은 항상 가장 먼저, 가장 중요하게 검사한다.**

- 앱 홈 URL 패턴: `https://toolpop.org/{app}` (예: `https://toolpop.org/pdf`, `https://toolpop.org/image`)
- 랜딩 홈 URL 패턴: `https://toolpop.org/{locale}` (예: `https://toolpop.org/ko`)
- 루트(`/`)는 로케일 감지 후 리다이렉트하므로 직접 측정하지 않는다.
...
(Image 앱 예시)
1. [홈] https://toolpop.org/image
2. https://toolpop.org/image/ko/resize
3. https://toolpop.org/image/ko/compress
...
...
- ❌ 루트(`/`) 측정 (로케일 감지 리다이렉트 — `/{locale}`을 대신 측정)
