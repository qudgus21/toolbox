...
**⚠️ 이 프로젝트의 배포 구조 이해 필수:**
- feature PR → `develop` 에 머지
- release 시 → cherry-pick → `release/vX.Y.Z` → `main` 에 머지 (PR base: main)
- `gh pr list --base main` 에는 release PR만 나타남. feature PR 번호로는 비교 불가.
- `git log origin/main..origin/develop` 은 커밋 해시 기준이라 cherry-pick된 PR을 오판함.

**올바른 방법: 최신 릴리즈 태그 시점 이후 develop에 머지된 PR을 찾는다.**
...
echo "마지막 릴리즈: $LAST_TAG ($TAG_TIME)"

# 2. 태그 이후 develop에 머지된 feature PR 조회 (release/* 제외)
gh pr list --base develop --state merged --json number,title,mergedAt --limit 50
# → mergedAt이 TAG_TIME 이후인 PR만 필터링
```

사용자에게 다음 형식으로 보여줍니다:

```
📦 배포 가능한 PR 목록: (v2.5.8 이후)

 #74  [fix] 모바일 다운로드 수정    (2026-03-28 머지)

배포할 PR 번호를 선택해주세요 (예: 74 또는 74,73):
```
...
- 현재 버전: `vX.Y.Z` — **반드시 `git tag --sort=-version:refname | head -1` 으로 확인** (`git describe`는 현재 브랜치 기준이라 오답 가능)
- 선택된 PR 수 & 요약
- 추천 버전 타입
...
- **⚠️ 미배포 PR 판단은 반드시 "최신 태그 이후 develop 머지 시점"으로** — `git log origin/main..origin/develop`, PR 번호 집합 비교 등은 이 프로젝트의 cherry-pick 릴리즈 구조에서 모두 오판함
- **⚠️ 현재 버전은 `git tag --sort=-version:refname | head -1`으로 확인** — `git describe --tags`는 체크아웃된 브랜치 기준이라 오답 가능
