...
```
Glob: src/lib/pdf/processors/__tests__/{도구명}.test.ts
```
...
```
src/lib/pdf/processors/__tests__/{도구명}.test.ts
```
...
```
Read: src/lib/pdf/processors/{도구명}.ts
```
...
```
Glob: src/app/pdf/[locale]/(tools)/_components/{도구명}-options.tsx
Glob: src/app/pdf/[locale]/(tools)/_components/{도구명}-*.tsx
```
...
```
Read: src/lib/pdf/processors/__tests__/helpers.ts
```
...
```bash
npx vitest run --reporter=verbose src/lib/pdf/processors/__tests__/{도구명}.test.ts 2>&1
```
...
특정 도구만:
```bash
npx vitest run --reporter=verbose src/lib/pdf/processors/__tests__/{도구}.test.ts 2>&1
```
...
1. 프로세서 코드를 읽는다:
   ```
   Read: src/lib/pdf/processors/{도구}.ts
   ```
...
3. 각 분기가 테스트되고 있는지 확인한다:
   ```
   Read: src/lib/pdf/processors/__tests__/{도구}.test.ts
   ```
...
```
Grep: "onChange" in src/app/pdf/[locale]/(tools)/_components/{도구}-options.tsx
```
...
```
Grep: "onExtractPagesChange|onRangesReorder|onRegisterSet" in src/app/pdf/[locale]/(tools)/_components/
```
