...
import type { PageSizeKey, Orientation, FileBreak } from "@/lib/pdf/processors/html-to-pdf";

export interface HtmlToPdfLabels {
...
interface HtmlToPdfOptionsProps {
  ...
  onFileBreakChange: (v: FileBreak) => void;
  onFileGapMmChange: (v: number) => void;
...
export function HtmlToPdfOptions({
  orientation,
  pageSize,
  marginMm,
  mergeAll,
  ...
  return (
    <div className="space-y-4">
      ...
      {/* ── File Break (only when mergeAll + multiple files) ── */}
      {fileCount > 1 && mergeAll && (
        <div className="space-y-3">
          <label className="text-sm font-semibold text-foreground">
            {labels.fileBreakLabel}
          </label>
          <div className="grid grid-cols-2 gap-2">
            {(["new-page", "continuous"] as const).map((val) => (
              <button
                key={val}
                type="button"
                onClick={() => onFileBreakChange(val)}
                className={cn(
              ...
              >
                {val === "new-page" ? labels.fileBreakNewPage : labels.fileBreakContinuous}
              </button>
            ))}
          </div>

          {/* Gap slider (only when continuous) */}
          <AnimatePresence>
            {fileBreak === "continuous" && (
              <motion.div
              ...
              >
                <div className="space-y-2 pt-1">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-xs font-medium text-foreground-muted tabular-nums">
                      {fileGapMm === 0 ? labels.marginNone : `${fileGapMm}${labels.marginUnit}`}
                    </span>
                  </div>
                  <input
                    type="range"
                    ...
                    step={GAP_STEP}
                    value={fileGapMm}
                    onChange={(e) => onFileGapMmChange(Number(e.target.value))}
                    className="w-full h-1.5 rounded-full appearance-none cursor-pointer bg-border accent-blue-500"
                  />
                </div>
              </motion.div>
