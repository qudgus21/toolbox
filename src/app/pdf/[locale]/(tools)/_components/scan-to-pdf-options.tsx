...
import type { PageSizeKey, Orientation, ScanColorMode } from "@/lib/pdf/processors/scan-to-pdf";

export interface ScanToPdfLabels {
...
interface ScanToPdfOptionsProps {
  ...
  onColorModeChange: (v: ScanColorMode) => void;
...
const COLOR_MODES: { value: ScanColorMode; labelKey: keyof ScanToPdfLabels }[] = [
  ...
  return (
    <div className="space-y-4">
      ...
      >
        ...
        >
          {autoEnhance && <Check className="h-3 w-3 text-white" />}
        </div>
        <div className="flex-1 min-w-0">
          <span className="text-sm font-semibold text-foreground">
            {labels.autoEnhanceLabel}
          </span>
          <p className="text-[11px] text-foreground-muted">
            {labels.autoEnhanceDesc}
          </p>
        </div>
      </button>
