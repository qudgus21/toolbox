...
export function StatsDisplay({ stats, labels, className }: StatsDisplayProps) {
  const entries = stats
    ? Object.entries(stats).filter((e): e is [string, string | number] => typeof e[1] !== "boolean")
    : [];

  return (
    <div className={cn(
      "rounded-2xl border border-border/50 bg-gradient-to-b from-background-elevated to-background overflow-hidden shadow-sm min-h-[200px]",
      className,
    )}>
      {entries.length > 0 ? (
