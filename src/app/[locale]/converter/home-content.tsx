...
import { tools, categories, categoryColors } from "@/lib/converter/tools";
import { converterToolIconMap } from "@/lib/converter/tool-icons";
import { getAppFavorites, toggleAppFavorite, reorderAppFavorites } from "@/lib/storage";
import { useTrack, converterEvents } from "@/lib/analytics";
...
const FAV_HINT_KEY = "converter-fav-hint-seen";
...
export function HomeContent({ dict, locale }: HomeContentProps) {
  ...
  const [favSlugs, setFavSlugs] = useState<string[]>(() => getAppFavorites("converter"));
  ...
  const refreshFavs = useCallback(() => {
    setFavSlugs(getAppFavorites("converter"));
  }, []);
  ...
  const handleToggleFav = useCallback((slug: string) => {
    const added = toggleAppFavorite("converter", slug);
    setFavSlugs(getAppFavorites("converter"));
  ...
  const renderToolCard = (tool: typeof tools[number], ctx: "grid" | "fav" = "grid") => {
    ...
    return viewMode === "grid" ? (
      <div className={cn("relative group/fav h-full")} onClick={handleToolClick}>
        <ToolCard
          data-card
          href={`/${locale}/converter/${tool.slug}`}
          linkComponent={Link}
          icon={tool.icon}
          title={toolDict.title}
          description={toolDict.description}
          toolIcon={converterToolIconMap[tool.slug] ? (() => { const Icon = converterToolIconMap[tool.slug]; return <Icon className="h-10 w-auto" />; })() : undefined}
      ...
      <div className={cn("relative group/fav")}>
        ...
        >
          {converterToolIconMap[tool.slug] ? (() => { const Icon = converterToolIconMap[tool.slug]; return <Icon className="h-7 w-auto shrink-0" />; })() : tool.emoji ? <span className="text-2xl shrink-0">{tool.emoji}</span> : null}
    ...
    <main className="pb-8">
      <Container size="full" className="max-w-screen-2xl">
        ...
        <div className="flex flex-wrap items-center justify-center gap-2.5 mb-6">
          ...
          {/* View Toggle */}
          <div className="hidden sm:flex items-center gap-1 ml-2 rounded-full border border-border dark:border-zinc-600 p-0.5">
            <button
