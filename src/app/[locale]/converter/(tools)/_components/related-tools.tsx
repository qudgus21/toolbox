...
import { tools, type ConverterToolDefinition } from "@/lib/converter/tools";
...
function getRelatedTools(currentSlug: string, recentSlugs: string[]): ConverterToolDefinition[] {
  const current = tools.find((t) => t.slug === currentSlug);
  if (!current) return tools.slice(0, 6);

  const used = new Set<string>([currentSlug]);
  const result: ConverterToolDefinition[] = [];
          ...
          return (
            <Link
              ...
              className={cn(
                ...
                "hover:border-emerald-500/30 hover:shadow-md hover:shadow-emerald-500/5 hover:-translate-y-0.5",
                "text-foreground-muted hover:text-foreground",
              )}
            >
              <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 transition-colors group-hover:bg-emerald-500/20">
