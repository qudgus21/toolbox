...
import { tools, categories, categoryColors } from "@/lib/calculator/tools";
import { calculatorToolIconMap } from "@/lib/calculator/tool-icons";
import { getAppFavorites, toggleAppFavorite, reorderAppFavorites } from "@/lib/storage";
import { useTrack, calculatorEvents } from "@/lib/analytics";
...
const FAV_HINT_KEY = "calculator-fav-hint-seen";
...
export function HomeContent({ dict, locale }: HomeContentProps) {
  ...
  const [favSlugs, setFavSlugs] = useState<string[]>(() => getAppFavorites("calculator"));
  ...
  const refreshFavs = useCallback(() => {
    setFavSlugs(getAppFavorites("calculator"));
  }, []);
  ...
  const handleToggleFav = useCallback((slug: string) => {
    const added = toggleAppFavorite("calculator", slug);
    setFavSlugs(getAppFavorites("calculator"));
    ...
    return viewMode === "grid" ? (
      <div className={cn("relative group/fav h-full")} onClick={handleToolClick}>
        <ToolCard
          ...
          toolIcon={calculatorToolIconMap[tool.slug] ? (() => { const Icon = calculatorToolIconMap[tool.slug]; return <Icon className="h-10 w-auto" />; })() : undefined}
      ...
      <div className={cn("relative group/fav")}>
        ...
        >
          {calculatorToolIconMap[tool.slug] ? (() => { const Icon = calculatorToolIconMap[tool.slug]; return <Icon className="h-7 w-auto shrink-0" />; })() : tool.emoji ? <span className="text-2xl shrink-0">{tool.emoji}</span> : null}
