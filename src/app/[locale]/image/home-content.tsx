...
import { tools, categories, categoryColors } from "@/lib/image/tools";
import { toolIconMap, categoryIconMap } from "@/lib/image/tool-icons";
...
const FAV_HINT_KEY = "image-fav-hint-seen";
...
export function HomeContent({ dict, locale }: HomeContentProps) {
  ...
  const refreshFavs = useCallback(() => {
    setFavSlugs(getAppFavorites("image"));
  }, []);
  ...
  const handleToggleFav = useCallback((slug: string) => {
    const added = toggleAppFavorite("image", slug);
    setFavSlugs(getAppFavorites("image"));
  ...
  const filteredTools = useMemo(() => {
    ...
    // comingSoon 도구를 맨 뒤로
    return [...result].sort((a, b) => (a.comingSoon ? 1 : 0) - (b.comingSoon ? 1 : 0));
