...
export async function generateMetadata({
  ...
  const title = `${dict.blog.title} ${dict.metadata.toolTitleSuffix}`;

  return {
    title,
    description: dict.blog.description,
    alternates: generateAlternates("blog", locales, locale, "en", "image"),
    openGraph: {
      title,
      description: dict.blog.description,
      url: `/image/${locale}/blog`,
      type: "website",
    },
  };
}
...
export default async function BlogPage({
  ...
  const localeArticles = getArticlesByApp("image")
        ...
        <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {localeArticles.map((article) => {
            ...
            return (
              <Link
                key={article.slug}
                href={`/image/${locale}/blog/${article.slug}`}
