...
import { tools, type ImageToolDefinition } from "@/lib/image/tools";
...
function getRelatedTools(currentSlug: string, recentSlugs: string[]): ImageToolDefinition[] {
  const current = tools.find((t) => t.slug === currentSlug);
  if (!current) return tools.filter((t) => !t.comingSoon).slice(0, 6);

  const used = new Set<string>([currentSlug]);
  const result: ImageToolDefinition[] = [];
  const activeTtools = tools.filter((t) => !t.comingSoon);

  // 1. Recently used tools first (up to 2)
  for (const slug of recentSlugs) {
    if (used.has(slug)) continue;
    const tool = activeTtools.find((t) => t.slug === slug);
    if (tool) {
      result.push(tool);
      used.add(slug);
    }
  ...
  // 2. Same category
  const sameCategory = activeTtools.filter(
  ...
  const others = activeTtools.filter((t) => !used.has(t.slug));
