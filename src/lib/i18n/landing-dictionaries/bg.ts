import type { LandingDictionary } from "../landing-config";

const dict: LandingDictionary = {
  meta: {
    title: "ToolPop — Безплатни онлайн инструменти",
    description: "Редактирайте, обединявайте и конвертирайте PDF файлове. Преоразмерявайте, компресирайте и трансформирайте изображения. Конвертирайте единици, цветове и формати на данни. Всичко безплатно, директно в браузъра.",
  },
  hero: {
    badge: "176+ безплатни онлайн инструмента",
    title: "Всички инструменти, от които се нуждаете, на едно място — ToolPop",
    titleAccent: "ToolPop",
    subtitle: "Работете с PDF файлове, изображения и конвертори директно в браузъра. Без инсталация, без регистрация.",
    searchPlaceholder: "Какъв инструмент търсите?",
  },
  apps: {
    pdf: { name: "PDF инструменти", description: "Обединяване, разделяне, конвертиране, редактиране и компресиране — всичко за PDF.", cta: "Разгледай PDF инструментите", toolCount: "39 инструмента" },
    image: { name: "Инструменти за изображения", description: "Преоразмеряване, изрязване, конвертиране, ефекти, генериране на QR кодове — всичко за изображения.", cta: "Разгледай инструментите за изображения", toolCount: "48 инструмента" },
    text: { name: "Текстови инструменти", description: "Трансформирайте, анализирайте, почистете, кодирайте и генерирайте текст моментално.", cta: "Текстови инструменти", toolCount: "44+ инструмента" },
    converter: { name: "Инструменти за конвертиране", description: "Конвертирайте единици, цветове, формати на данни, дати и още.", cta: "Конвертори", toolCount: "45 инструмента" },
  },
  popularTools: { sectionTitle: "Популярни инструменти", sectionSubtitle: "Започнете веднага с най-използваните инструменти", viewAll: "Виж всички" },
  stats: { tools: "176+", toolsLabel: "Безплатни инструмента", languages: "45", languagesLabel: "Езика", users: "100%", usersLabel: "В браузъра", price: "0 лв", priceLabel: "Безплатно завинаги" },
  trust: {
    sectionTitle: "Защо ToolPop?",
    sectionSubtitle: "Сигурност и удобство без компромиси",
    browserBased: "Обработка в браузъра",
    browserBasedDesc: "Файловете ви никога не напускат устройството. Всичко работи в браузъра — бързо и сигурно.",
    free: "Напълно безплатно",
    freeDesc: "Използвайте всеки инструмент колкото пъти искате, без ограничения. Без скрити такси.",
    private: "Поверителност на първо място",
    privateDesc: "Файловете остават на вашето устройство. Без качване на сървъри — данните ви си остават ваши.",
    noSignup: "Без регистрация",
    noSignupDesc: "Без акаунт, без имейл. Просто отворете инструмент и започнете работа.",
  },
  cta: { title: "Готови ли сте да започнете?", subtitle: "Без инсталация, без регистрация, без плащане — само едно кликване.", button: "Разгледай всички инструменти" },
  common: { shareTitle: "Споделяне", shareSubtitle: "Разкажете на приятелите си за ToolPop!", shareCopyLink: "Копирай линк", shareCopied: "Копирано!" },
  footer: { apps: "Инструменти", company: "Компания", about: "За нас", contact: "Контакт", faq: "Често задавани въпроси", blog: "Блог", legal: "Правна информация", privacy: "Политика за поверителност", terms: "Условия за ползване", copyright: "© 2026 ToolPop. All rights reserved.", tagline: "Безплатни онлайн инструменти за PDF, изображения и още. Без регистрация.", madeWith: "Направено с любов за всички" },
  search: { noResults: "Няма намерени резултати" },
  nav: { pdf: "PDF", image: "Изображение", text: "Текст", converter: "Конвертор" },
};
export default dict;
